...
async function main() {
  ...
  try {
    ...
    // 5. Скроллим к кнопке копирования ссылки и кликаем
    await page.waitForSelector('.share-link-copy', { timeout: 10000 });
    await page.locator('.share-link-copy').scrollIntoViewIfNeeded();
    await page.waitForTimeout(500);
    await page.click('.share-link-copy');
    await page.waitForTimeout(500);

    console.log('Готово! Поиск → НДА → Копирование ссылки — выполнено.');
  } catch (e) {
